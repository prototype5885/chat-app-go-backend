...
func saveAvatar(file multipart.File) (string, error) {
	...
	if err == nil { // avatar with same hash already exist, no need to write to disk
		slog.Debug(fmt.Sprintf("Avatar %s has been generated by a different user just now, no need to save", fileName))
		return fileName, nil
	...
	slog.Debug(fmt.Sprintf("Saved avatar %s", fileName))
	return fileName, nil
}
...
func generateResizedAvatar(name string, size int) error {
	...
	slog.Debug(fmt.Sprintf("Saved resized size %d of avatar %s", size, name))
	return nil
}
